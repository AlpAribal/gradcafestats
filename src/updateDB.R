...
repeat {
  ...
  # Merge results
  results <- cbind(subId, submissionDate = submissions$`Date Added`
                   , institution = submissions$Institution, prog, notif[, .(notif_result, notif_method, notif_date)]
                   , submissions$St1, notif[, .(gpa, gre_v, gre_q, gre_w, gre_subject)], notes = submissions$Notes)
