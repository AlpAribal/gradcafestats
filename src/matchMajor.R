...
if (!FULL_DB_MATCH) {
    matches <- fread(input = "../data/matchMajors.csv", sep = ";", header = T, quote = "")
    
    uniqueMajors <- unique(setdiff(uniqueMajors, matches$major))
}

majors_all <- fread(input = "../data/majors.csv", header = T, quote = "\"")

if (TEST) {
    # majors_all <- majors_all[(nrow(majors_all)-2):nrow(majors_all)] majors_all <-
    # majors_all[32]
}
...
if (TEST) {
    test_res <- res[rSum > 0]
    test_res <- merge(majors_all[, .(majorId, major)], test_res, by.y = "matched_majorId", 
        by.x = "majorId")
    test_res <- test_res[majorId > 0]
    unmatched <- merge(res[rSum == 0, .(major)], submissions[, .(major)], by = "major", 
        all = F)
    unmatched <- unmatched[, .N, by = list(major)]
    multimatched <- merge(res[rSum > 1, .(major)], submissions[, .(major)], by = "major", 
        all = F)
    ...
    write.table(x = write_res, file = "../data/matchMajors.csv", append = !FULL_DB_MATCH, 
