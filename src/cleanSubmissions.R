...
submissions[is.na(sem), sem := ifelse(month(submissionDate) < 9
                                      , paste0("F",str_sub(year(submissionDate), start = 3))
                                      , paste0("F",str_sub(year(submissionDate)+1, start = 3))
)]
...
submissions[gre_v > 800 | (gre_v > 170 & gre_v < 200) | gre_v < 130, gre_v := NA]
...
submissions[gre_q > 800 | (gre_q > 170 & gre_q < 200) | gre_q < 130, gre_q := NA]
submissions[gre_q >= 200, gre_q := concQuant[findInterval(gre_q, concQuant$PriorScale), CurrentScale]]
...
cleanSubmissions <- merge(cleanSubmissions, insts_all[instId > 0]
                          , by.x = "matched_instId", by.y = "instId", all = F)
...
cleanSubmissions <- merge(cleanSubmissions, majors_all[majorId > 0]
                          , by.x = "matched_majorId", by.y = "majorId", all = F)
